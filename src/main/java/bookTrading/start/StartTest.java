...
//            o2a通信机制使得由in-process接口启动的外部应用程序和agent之间的交互成为可能。
//            本质上是一个同步FIFO队列。
...
//            if里面是在原有main容器中启动agent
            ...
            System.out.println("Launching the buyer agent on the main container ...");
            AgentController buyer = mc.createNewAgent("buyer", "bookTrading.buyer.BookBuyerAgent", new Object[0]);
            buyer.start();

            System.out.println("Launching the seller agent on the main container ...");
            AgentController seller = mc.createNewAgent("seller", "bookTrading.seller.BookSellerAgent", new Object[0]);
            seller.start();
